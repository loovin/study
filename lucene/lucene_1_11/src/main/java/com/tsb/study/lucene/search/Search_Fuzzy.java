...
public class Search_Fuzzy {
	...
	public void searchByFuzzy() {
		System.out.println("============== search by fuzzy ===========");
		IndexSearcher searcher = getSearcher();
		
		//允许一个字符不匹配
		int prefixLength = 2;//表示前2个字母必须匹配
		float minimumSimilarity = 0.6f;//值越大则越像，也就是匹配要求越严格
		FuzzyQuery query = new FuzzyQuery(new Term("name","mihe"),minimumSimilarity,prefixLength);
	...
	public static void main(String[] args) {
		Search_Fuzzy indexUtil = new Search_Fuzzy();
		indexUtil.index();
		
		indexUtil.searchByFuzzy();
	}
